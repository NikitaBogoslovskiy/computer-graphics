...
public:
	...
	void fillBuffers(std::vector<Mesh*>& meshes, Torch* torch, Eigen::Matrix4f& vp, ImVec3& cam_pos, ImVec3& cam_dir);
	void draw(ImDrawList* draw_list, ImVec2& offset);
protected:
	void calculateColors(Mesh* mesh, Torch* torch, std::vector<ImVec4>& colors);
	void processPolygon(Mesh* mesh, Polygon& poly, std::vector<ImVec4>& colors, Eigen::Matrix4f& vp, ImVec3& cam_pos);
	void interpolateLine(Eigen::Vector4f& p0_3d, Eigen::Vector4f& p1_3d, LightRegion& polygonRegion, ImVec2& size, ImVec4 p0_color, ImVec4 p1_color, float p0_depth, float p1_depth);
};
