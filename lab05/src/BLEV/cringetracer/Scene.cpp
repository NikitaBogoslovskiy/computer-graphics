...
Scene::Scene()
{
	std::map<std::string, Material*> mtlLib{ // please move it so it become accessible to the interface
			{"brick", new Material()},
	...
	bodies.push_back(new Plane(HVec<double> {0.0, 0.5, 0.0},
		HVec<double> { M_PI * 0.5, 0.0, 0.0 },
		HVec<double> {4.0, 4.0, 4.0},
		ImVec3{ 100.0f / 255.f, 100.0f / 255.f, 100.0f / 255.f }));
	//lol

	bodies.at(0)->SetMaterial(mtlLib["lightBlueMatte"]);
	bodies.at(1)->SetMaterial(mtlLib["mirror"]);
	//bodies.at(0)->SetMaterial(mtlLib["mirror"]);
	//bodies.at(1)->SetMaterial(mtlLib["lightBlueMatte"]);
