...
Mesh* rotationBodyX(std::vector<Point>& points, int iterNumber)
{
	...
	return rotationBody(points, iterNumber, rmat);
}
...
Mesh* rotationBody(std::vector<Point>& points, int iterNumber, Eigen::Matrix<float, 4, 4>& rmat)
{
	...
	pointsMatrix.resize(4, points.size());
	for (size_t i = 0; i < points.size(); i++) {
		pointsMatrix(0, i) = points[i].get().y;
		pointsMatrix(1, i) = points[i].get().x;
		pointsMatrix(2, i) = 0;
		pointsMatrix(3, i) = 1;
	}
	...
	for (int i = 0; i < iterNumber; ++i)
	{
		for (size_t j = 0; j < pointsNumber; ++j) {
			m->add_point(ImVec3(pointsMatrix(0, j), pointsMatrix(1, j), pointsMatrix(2, j)));
			if (j != pointsNumber - 1)
				m->add_polygon(Polygon{ j + i * pointsNumber, j + 1 + i * pointsNumber, j + 1 + ((i + 1) % iterNumber) * pointsNumber, j + ((i + 1) % iterNumber) * pointsNumber });
		}
