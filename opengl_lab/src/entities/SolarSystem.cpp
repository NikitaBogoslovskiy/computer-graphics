...
void SolarSystem::PrepareData(const glm::vec3& center)
{
    ...
    for (unsigned int i = 0; i < planets_number; i++)
    {
        ...
        pp->movement_speed = 1.0 + static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / 8.0));
        pp->scale_factor = 0.05 + static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / 0.009));
        pp->selfrotation_speed = 1.0 + static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / 8.0));
...
void SolarSystem::Draw(float time_coefficient, const glm::mat4& view, const glm::mat4& projection)
{
    ...
    for (unsigned int i = 0; i < planets_number; i++)
    {
        ...
        model = glm::rotate(model, self_rot_angle, glm::vec3(0.0f, 1.0f, 0.0f));
        model_matrices[i] = model;
    }
    ...
    glDrawElementsInstanced(GL_TRIANGLES, static_cast<unsigned int>(planet->mVs.size()), GL_UNSIGNED_INT, 0, planets_number);
    glBindVertexArray(0);
    glUseProgram(0);
}
